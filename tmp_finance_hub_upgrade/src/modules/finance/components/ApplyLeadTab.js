  ...
  <section className="finance-section">
    ...
    <form className="finance-form" onSubmit={onSubmit}>
      ...
      <div className="finance-security-note">
        <strong>Document safety:</strong> Upload only documents needed for loan processing. Do not upload passwords,
        unrelated personal files, ATM PINs, net-banking credentials, or full card details.
      </div>
    ...
    {state.success ? (
      <div className="finance-success-card">
        ...
        <div className="finance-success-actions">
          <a href={`https://wa.me/91${String(form.phone || "").replace(/\D/g, "")}?text=${encodeURIComponent("Hi, I submitted a loan enquiry in Nila Finance Hub. Please help me with the next step.")}`} target="_blank" rel="noreferrer">
            WhatsApp Support
          </a>
          <span>Next: upload pending documents and track application status.</span>
        </div>
      </div>
    ) : null}

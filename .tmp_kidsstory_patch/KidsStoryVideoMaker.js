    ...
    return {
      ...
      visualPrompt: [
        ...
        `characters visible on screen: ${sceneCharacters
          .map((character) => `${character.name}, ${character.role}, ${character.appearance || "cartoon child character"}`)
          .join("; ")}`,
...
const KidsStoryVideoMaker = () => {
  ...
  const handleRenderVideo = async () => {
    ...
    setError("");
    setMessage("Rendering real cartoon characters with spoken dialogue, scene visuals, music, and subtitles...");
    setIsRendering(true);
    startRenderProgress();

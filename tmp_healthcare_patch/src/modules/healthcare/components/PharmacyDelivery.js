...
const getMedicineInfo = (medicine = {}) => {
  ...
  return (
    medicine.info ||
    MEDICINE_INFO[normalizedName] || {
      purpose: `${medicine.name || "This medicine"} doctor/pharmacist advice anusarich use cheyyenda medicine aanu. Use depends on condition and dose.`,
      ingredients: medicine.composition || medicine.ingredients || "Exact ingredients brand/strip label anusarich verify cheyyuka.",
      warning: medicine.requiresPrescription ? "Prescription required. Doctor advice illathe use cheyyaruthu." : "Label instructions and pharmacist advice follow cheyyuka.",
    }
  );
};
...
const PharmacyDelivery = ({ medicines, loading, orders, onCreateOrder, onVerifyPayment, onUpdateOrderStatus }) => {
  ...
  const enrichedMedicines = useMemo(() => {
    return (medicines || []).map((medicine) => ({ ...medicine, info: getMedicineInfo(medicine) }));
  }, [medicines]);

  const filteredMedicines = useMemo(() => {
    if (!query.trim()) return enrichedMedicines;
  ...
  const addToCart = (medicine) => {
    ...
    setCart((previous) => {
      ...
      return [...previous, { ...medicine, pharmacyId: selectedPharmacy.id, pharmacyName: selectedPharmacy.name, quantity: 1 }];
    });
  ...
  const placeOrder = async (event) => {
    ...
    try {
      ...
      setPaymentForm({ fullName: "", phone: "", address: "", paymentMethod: "upi" });
    ...
    <section className="healthcare-section">
      ...
      <div className="healthcare-selection-panel">
        ...
        <div className="healthcare-partner-summary">
          ...
          <span>{selectedPharmacy.area} | Delivery: {selectedPharmacy.deliveryEta}</span>
        </div>
      </div>
      ...
      {query.trim() ? (
        <div className="healthcare-info-card">
          ...
          {filteredMedicines.slice(0, 3).map((medicine) => (
            <div key={medicine.id} className="healthcare-info-row">
              <span>{medicine.name}</span>
              <p>{medicine.info.purpose}</p>
              <small>Ingredients: {medicine.info.ingredients}</small>
            </div>
          ))}
          {filteredMedicines.length === 0 ? <p>No exact medicine found. Upload prescription or contact pharmacy support.</p> : null}
        </div>
      ) : null}
      ...
      <div className="healthcare-pharmacy-grid">
        <div className="healthcare-upload-card">
          ...
          <button type="button" className="healthcare-primary-button" onClick={verifyPrescription} disabled={verifying || !prescriptionFile}>
          ...
          <div className="healthcare-pill-row">
            ...
            <button type="button" className="healthcare-secondary-button" onClick={() => setShowCart(true)}>Open Cart ({cart.length})</button>
          </div>
        </div>
            ...
            {filteredMedicines.map((medicine) => (
              <article key={medicine.id} className="healthcare-medicine-card healthcare-info-enabled-card">
                <div>
                  ...
                  <span>{medicine.category} | {formatCurrency(medicine.price)}</span>
                  <p className="healthcare-brief-info">{medicine.info.purpose}</p>
                  <small>Ingredients: {medicine.info.ingredients}</small>
                  {medicine.requiresPrescription ? <span className="healthcare-warning-text">Prescription required</span> : <span className="healthcare-success-text">No prescription required</span>}
                  <small className="healthcare-warning-text">{medicine.info.warning}</small>
                </div>

                <button type="button" className="healthcare-primary-button" onClick={() => addToCart(medicine)} disabled={medicine.requiresPrescription && !prescriptionVerified}>
                  Add To Cart
                </button>
              </article>
            ))}
      ...
      {lastCreatedOrder ? (
        <div className="healthcare-inline-alert" role="status">
          Latest order: {lastCreatedOrder.id} ({lastCreatedOrder.orderStatus || "placed"}) payment {lastCreatedOrder.paymentStatus || "pending"}.
        </div>
      ) : null}
          ...
          <article key={order.id} className="healthcare-record-item">
            ...
            <div className="healthcare-record-actions">
              {NEXT_ORDER_STATUS[order.orderStatus || "placed"] ? (
                <button type="button" className="healthcare-secondary-button" onClick={() => onUpdateOrderStatus?.(order.id, NEXT_ORDER_STATUS[order.orderStatus || "placed"])}>
        ...
        <div className="healthcare-modal-overlay" role="dialog" aria-modal="true" aria-label="Pharmacy cart">
          <div className="healthcare-modal">
            <div className="healthcare-modal-header">
              ...
              <button type="button" className="healthcare-close-button" onClick={() => setShowCart(false)}>Close</button>
            </div>
            ...
            <div className="healthcare-cart-footer">
              ...
              <button type="button" className="healthcare-primary-button" onClick={() => setShowPayment(true)} disabled={cart.length === 0}>Continue To Payment</button>
            </div>
          </div>
        </div>
      ) : null}
      ...
      {showPayment ? (
        <div className="healthcare-modal-overlay" role="dialog" aria-modal="true" aria-label="Payment">
          <div className="healthcare-modal">
            <div className="healthcare-modal-header">
              <h3>Payment</h3>
              <button type="button" className="healthcare-close-button" onClick={() => setShowPayment(false)}>Close</button>
            </div>
            ...
            <form className="healthcare-form-grid" onSubmit={placeOrder}>
              <label className="healthcare-field">
                <span>Pharmacy</span>
                <select value={selectedPharmacyId} onChange={(event) => setSelectedPharmacyId(event.target.value)}>
                  {PHARMACY_PARTNERS.map((pharmacy) => <option key={pharmacy.id} value={pharmacy.id}>{pharmacy.name} - {pharmacy.area}</option>)}
                </select>
              </label>

              <label className="healthcare-field">
                <span>Full Name</span>
                <input type="text" value={paymentForm.fullName} onChange={(event) => setPaymentForm((previous) => ({ ...previous, fullName: event.target.value }))} required />
              </label>

              <label className="healthcare-field">
                <span>Phone</span>
                <input type="tel" value={paymentForm.phone} onChange={(event) => setPaymentForm((previous) => ({ ...previous, phone: event.target.value }))} required />
              </label>

              <label className="healthcare-field healthcare-field-full">
                <span>Delivery Address</span>
                <input type="text" value={paymentForm.address} onChange={(event) => setPaymentForm((previous) => ({ ...previous, address: event.target.value }))} required />
              </label>

              <label className="healthcare-field">
                <span>Payment Method</span>
                <select value={paymentForm.paymentMethod} onChange={(event) => setPaymentForm((previous) => ({ ...previous, paymentMethod: event.target.value }))}>
              ...
              <div className="healthcare-modal-actions">
                <button type="button" className="healthcare-secondary-button" onClick={() => setShowPayment(false)}>Back</button>
                <button type="submit" className="healthcare-primary-button" disabled={placingOrder}>{placingOrder ? "Processing..." : `Pay ${formatCurrency(cartTotal)}`}</button>
              </div>
            </form>
          </div>
        </div>

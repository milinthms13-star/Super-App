...
import LinkUpProChat from './LinkUpProChat';
...
const Messaging = () => {
  ...
  const persistOutbox = useCallback((nextQueue) => {
     ...
     outboxRef.current = safeQueue;
     setOutboxEntries(safeQueue);
  }, [currentUserId]);
  ...
  const retryQueuedMessage = useCallback(async (queuedMessage, { manual = false } = {}) => {
    ...
    const delaySeconds = Math.min(60, 2 ** Math.max(0, nextAttemptCount - 1));
  ...
  const flushMessageOutbox = useCallback(async () => {
    ...
    try {
      const queuedMessages = [...outboxRef.current]
        ...
        .filter((queuedMessage) => {
          if (!queuedMessage.nextAttemptAt) {
            return true;
          }
          return Date.now() >= new Date(queuedMessage.nextAttemptAt).getTime();
        })
        .sort(
          (first, second) =>
            new Date(first.createdAt || 0).getTime() - new Date(second.createdAt || 0).getTime()
  ...
  return (
    ...
    >
      ...
      <div className="messaging-grade">
        <div className="messaging-grade-left">
          <strong>Messaging Health:</strong>
          <span className="messaging-stars">{'★'.repeat(Math.round(messagingScore / 2))}{'☆'.repeat(5 - Math.round(messagingScore / 2))}</span>
          <span className="messaging-score">{(messagingScore / 2).toFixed(1)}/5</span>
          {lastOutboxFlushAt && (
            <span className="messaging-flush-at">Last flush: {new Date(lastOutboxFlushAt).toLocaleTimeString()}</span>
          )}
        </div>
        <div className="messaging-grade-right">
          <button
            className="btn-optimize"
            onClick={async () => {
              if (isOptimizingMessaging) return;
              setIsOptimizingMessaging(true);
              try {
                ...
                if (socket && socket.connect && !socket.connected) {
                  try {
                    ...
                    console.warn('Socket reconnect attempt failed', e);
                  }
                }

                try {
                  await flushMessageOutbox();
                  setLastOutboxFlushAt(new Date().toISOString());
                } catch (e) {
                  console.warn('Outbox flush during optimize failed', e);
                }
              } finally {
                setIsOptimizingMessaging(false);
                setTimeout(() => computeMessagingDiagnostics(), 300);
              }
            }}
            title="Attempt conservative optimizations to improve reliability"
          >
            {isOptimizingMessaging ? 'Optimizing...' : 'Make it 5/5'}
          </button>
        </div>
      </div>
      {outboxEntries.length > 0 && (
        <section className="messaging-queue-panel">
          <div className="messaging-queue-header">
            <div>
              <strong>{outboxEntries.length} Queued message{outboxEntries.length === 1 ? '' : 's'}</strong>
              <p className="messaging-queue-subtitle">
                Messages waiting to be delivered or retrying automatically.
              </p>
            </div>
            <button
              className="btn-retry-queue"
              onClick={async () => {
                await flushMessageOutbox();
                setLastOutboxFlushAt(new Date().toISOString());
                computeMessagingDiagnostics();
          ...
          <ul className="messaging-queue-list">
            {outboxEntries.map((entry) => {
              const nextAttemptAt = entry.nextAttemptAt ? new Date(entry.nextAttemptAt) : null;
              const retryIn = nextAttemptAt ? Math.max(0, Math.round((nextAttemptAt.getTime() - Date.now()) / 1000)) : 0;
              const queueStatus = entry.permanentFailure
                ? 'Permanent failure'
                : entry.errorMessage
                ? 'Retry scheduled'
                : 'Queued';
              return (
                <li key={entry.clientMessageId} className="messaging-queue-item">
                  <div className="queue-item-main">
                    <span className="queue-item-title">{entry.content || entry.messageType || 'Pending message'}</span>
                    <span className="queue-item-meta">
                      {queueStatus} · attempt {entry.attemptCount || 1}/{MESSAGE_OUTBOX_RETRY_LIMIT}
                      {nextAttemptAt && !entry.permanentFailure ? ` · retry in ${retryIn}s` : ''}
                    </span>
                  </div>
                  <div className="queue-item-actions">
                    {entry.errorMessage && <span className="queue-item-error">{entry.errorMessage}</span>}
                    <button
                      className="btn-retry-message"
                      onClick={() => handleRetryMessage(entry)}
                      title="Retry this queued message"
                    >
                      Retry
                    </button>
                  </div>
      ...
      <div className="messaging-layout">
        <div className="messaging-sidebar linkup-chat-rail">
          <div className="sidebar-header">
            <div className="sidebar-tabs">
              ...
              <button
                className={`tab-btn ${activeTab === 'prochat' ? 'active' : ''}`}
                onClick={() => setActiveTab('prochat')}
                type="button"
              >
                <span className="tab-icon">PRO</span>
                <span>Pro Chat</span>
              </button>
              <button
        ...
        <div className="messaging-main">
          {activeTab === 'prochat' ? (
            <LinkUpProChat currentUser={currentUser} />
          ) : activeTab === 'chatrooms' ? (

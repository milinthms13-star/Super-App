        ...
        <div className="jobs-table">
          {jobs.slice(0, 3).map(job => (
            <div key={job.id} className="job-row">
              <div className="job-info">
                <h4>{job.title}</h4>
                <p>{job.location} • {job.applications || Math.floor(Math.random() * 20)} applications</p>
              </div>
              <div className="job-status">
                <span className="status active">Active</span>
              </div>
  ...
  return (
    <div className="job-portal-shell">
      <div className="job-portal-nav">
        ...
        <button
          className={`nav-tab ${activeTab === 'profile' ? 'active' : ''}`}
          onClick={() => { setActiveTab('profile'); setShowProfileBuilder(true); }}
        >
          👤 Profile
        </button>
        <button
          className={`nav-tab ${activeTab === 'employer' ? 'active' : ''}`}
          onClick={() => { setActiveTab('employer'); setShowEmployerDashboard(true); }}
        >
          🏢 Employer
        </button>
      </div>
      ...
      <div className="job-portal-content">
        {activeTab === 'home' && !selectedCategory && !showProfileBuilder && !showEmployerDashboard && renderHomeScreen()}
        {selectedCategory && renderCategoryView()}
        {showProfileBuilder && renderProfileBuilder()}
        {showEmployerDashboard && renderEmployerDashboard()}
      </div>

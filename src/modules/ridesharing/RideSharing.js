...
const RideSharing = () => {
  const { user } = useApp();
  const [activeMode, setActiveMode] = useState('rider'); // 'rider', 'driver', 'admin'
  const [currentScreen, setCurrentScreen] = useState('home'); // home, book, tracking, profile, driver-dashboard, admin
  ...
  const loadUserData = async () => {
    try {
      setLoading(true);
      if (activeMode === 'rider') {
        // Load rider profile and ride history
        ...
        // Load driver profile and status
      ...
      <div className="ride-tracking">
        ...
        <div className="ride-status-card">
          <div className="status-info">
            <h3>{currentRide.status.replace('_', ' ').toUpperCase()}</h3>
            <p>Ride ID: {currentRide.rideId}</p>
          </div>
        ...
        <div className="ride-details">
          <div className="location-info">
            ...
            <div className="drop">
              <span className="location-icon">🎯</span>
              <span>{currentRide.destination.address}</span>
            </div>
          </div>

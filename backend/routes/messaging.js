...
router.put('/messages/:messageId/important', authenticate, attachMessagingUser, async (req, res, next) => {
  try {
    ...
    // Check if user already marked as important
    const userIndex = (message.markedAsImportantBy || []).findIndex((userId) =>
      userId.equals(req.user._id)
    );

    if (userIndex > -1) {
      // Remove from important
      message.markedAsImportantBy.splice(userIndex, 1);
    } else {
      // Add to important
      if (!message.markedAsImportantBy) {
        message.markedAsImportantBy = [];
      }
      message.markedAsImportantBy.push(req.user._id);
    }

    await message.save();
    await message.populate('markedAsImportantBy', '_id');

    logger.info(`Message marked as important: ${messageId} by ${req.user._id}`);
    res.json({ message, isImportant: userIndex === -1 });
  } catch (err) {
    next(err);
  }
});
...
router.put('/contacts/:contactUserId/favorite', authenticate, attachMessagingUser, async (req, res, next) => {
  try {
    ...
    logger.info(`Contact favorite toggled for ${contactUserId} by ${req.user._id}: ${contact.isFavorite}`);
    res.json({ contact });
  } catch (err) {

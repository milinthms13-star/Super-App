/**
 * backend/services/videoStudioRealCartoonRenderer.js
 *
 * Real cartoon renderer with FREE TTS integration using Google Cloud TTS free tier.
 * This renders cartoon character visuals with animated mouth movements and spoken dialogue.
 *
 * Free APIs used:
 * - Google Cloud TTS (1M characters/month free)
 * - FFmpeg (open source)
 * - Sharp (open source)
 */
...
// Free TTS Integration - Google Cloud TTS (1M chars/month free)
const { initializeGoogleTTS } = require('./googleTTSCredentialLoader');
let ttsClient = null;

const initGoogleTTS = () => {
  if (ttsClient) return ttsClient;
  ttsClient = initializeGoogleTTS();
  return ttsClient;
};

const synthesizeSpeech = async (text, voiceType = 'child-friendly') => {
  const client = initGoogleTTS();
  if (!client) return null;

  try {
    // Free Google TTS voices suitable for kids
    const voiceMap = {
    ...
    const voice = voiceMap[voiceType] || voiceMap['child-friendly'];
    
    const request = {
      input: { text: String(text || '').slice(0, 5000) }, // Free tier limit per request
      voice: voice,
      audioConfig: { 
        audioEncoding: 'MP3',
        speakingRate: 1.0,
        pitch: 2.0 // Higher pitch for kid-friendly voice
      },
    };
    ...
    console.warn('TTS synthesis failed, using silent audio:', error.message);
    return null;
  }
};
...
const createSceneVideo = async ({ project, scene, index, workDir }) => {
  ...
  const audioFile = path.join(workDir, `scene-${index}-audio.mp3`);
  ...
  const dialogueText = Array.isArray(scene.spokenLines) && scene.spokenLines.length
    ? scene.spokenLines.map(line => `${line.speaker} says: ${line.text}`).join('. ')
    : scene.dialogue || scene.description || scene.title;

  // Try to generate speech using FREE Google TTS
  const audioBuffer = await synthesizeSpeech(dialogueText, project.voiceType);
  
  if (audioBuffer) {
    // Use real TTS audio
    await fs.writeFile(audioFile, audioBuffer);
    console.log(`✓ Generated speech for scene ${index + 1} using FREE Google TTS`);
  } else {
    ...
    console.log(`⚠ Using silent audio for scene ${index + 1} (TTS not available)`);
  }

  // Create video with mouth animation
  await runFfmpeg(
    ffmpeg()
...
async function renderRealCartoonProject(project = {}) {
  ...
  const normalizedProject = {
    ...
    voiceType: project.voiceType || 'child-friendly'
  };

  console.log(`🎬 Starting cartoon render for project ${projectId} with ${project.scenes.length} scenes`);
  
  const sceneVideos = [];
  for (let i = 0; i < project.scenes.length; i += 1) {
    console.log(`  Rendering scene ${i + 1}/${project.scenes.length}...`);
    sceneVideos.push(await createSceneVideo({ 
  ...
  console.log(`  Concatenating ${sceneVideos.length} scene videos...`);
  await concatVideos(sceneVideos, outputPath, workDir);

  console.log(`✓ Cartoon video rendered successfully: ${outputPath}`);
  ...
  return {
    ...
    usedSpokenDialogue: ttsClient !== null,
    scenesCount: project.scenes.length,
    audioType: ttsClient ? 'Google Cloud TTS (FREE)' : 'Silent audio fallback'
  };
}

module.exports = { renderRealCartoonProject, synthesizeSpeech };
